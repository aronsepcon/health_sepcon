...
class PreviewVaccine extends StatefulWidget {
  ...
  const PreviewVaccine({super.key,required this.file,required this.titlePage});
              ...
              children: [
                ...
                GestureDetector(
                  onTap: () async {
                    ...
                    List<String> tempList = await localStore.fetchPathsFileByTypeDocument(Constants.DOCUMENT_VACUUM);

                    tempList.add(viewPhoto);
                    log("LIST ${tempList.length}");
                    bool result = await localStore.saveFilePaths(Constants.DOCUMENT_VACUUM,tempList);

                    if(result){
                      Navigator.pushReplacement(
                          context ,
                          MaterialPageRoute(
                              builder: (_) => CollectPhotoVacuum()));
                    }
